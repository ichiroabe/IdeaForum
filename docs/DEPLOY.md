...
IdeaForum専用に作成したDB。既存WordPressのDB(`LAA1700269-irxlcy` /
`mysql80-2.lolipop.lan`)とは**別サーバー・別パスワード**なので取り違えないこと。

| 項目 | 値 |
|---|---|
| データベース名 | `LAA1700269-ideaforum` |
| ホスト | `mysql84-1.lolipop.lan` |
| バージョン | MySQL 8.4 |
| ユーザー名 | `LAA1700269` |
| パスワード | このDBの作成時に発行された値 |

phpMyAdmin で**このDBを選択した状態**にしてから、`sql/schema.sql` の内容をそのまま実行する。
(サーバー全体のSQL画面で実行すると `#1046 データベースが選択されていません` になる)
...
5. 画面を再読み込みするとヘッダーに「管理」リンクが出る
   (権限は毎回DBから読むので、再ログインは不要)
